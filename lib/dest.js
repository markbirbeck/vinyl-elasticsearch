  ...
  return h.pipeline(function(stream) {
    ...
    return _stream
      ...
      .consume((err, batch, push, next) => {
        ...
        let bulkActions = {body: []};

        batch.forEach(file => {
          bulkActions.body.push({
            index: {
              _index: file.index,
              _type: file.type,
              _id: file.id
          ...
          bulkActions.body.push(file.body);
        });
        ...
        let timeKey = `[vinyl-elasticsearch]: written ${batch.length} records on ${_opt.host}: took`;

        console.time(timeKey);
        client.bulk(bulkActions, function(error, response) {
          if (error) {
