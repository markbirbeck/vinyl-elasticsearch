...
module.exports = (glob, _opt) => {
  ...
  return h.pipeline(stream => {
    ...
    .map(file => ({
        action: glob.action || file.action || opt.action,
        index: glob.index || file.index || opt.index,
        type: glob.type || file.type || opt.type || file.base,
    ...
    .map(batch => {
      ...
      batch.forEach(file => {
        ...
        switch (action) {
          ...
          /**
           * For an index or create action the body is the document:
           */
          ...
          /**
           * There is no body for a delete action:
           */
          ...
          /**
           * For an update action the body is an object that contains a 'doc'
           * property which references the document:
           */

          case 'update':
            bulkActions.push({doc: file.body});
            break;
        }
      });
